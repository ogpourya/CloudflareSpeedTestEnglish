...
:: If nowip_hosts.txt does not exist, this is the first run.
if not exist "nowip_hosts.txt" (
    echo This script runs CFST to get the fastest IP and replace the Cloudflare CDN IP in Hosts.
    echo Before use, please read: https://github.com/XIU2/CloudflareSpeedTest/issues/42#issuecomment-768273768
    echo.
    echo On first use, change all Cloudflare CDN IPs in Hosts to the same IP.
    set /p nowip="Enter that Cloudflare CDN IP and press Enter (this step will not be needed later):"
    echo !nowip!>nowip_hosts.txt
    echo.
)

:: Read the current Cloudflare CDN IP used by Hosts from nowip_hosts.txt.
